...
export default function NotificationsMenu() {
  ...
  useEffect(() => {
    ...
    try {
      console.log("[NotificationsMenu] mounted", {
        tokenPresent:
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      try {
        console.log(
          "[NotificationsMenu] fetch status",
          res.status,
          res.statusText,
          {
      ...
      if (res.status === 200) {
        ...
        try {
          console.log(
            "[NotificationsMenu] fetch /api/notifications response",
            data
          );
        } catch (e) {}
        ...
        console.warn(
          "[NotificationsMenu] unexpected response",
          res.status,
          res.statusText
        );
        try {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      <div className="p-0.5 rounded-full bg-linear-to-r from-[#5D5FEF] via-[#8A5AF1] to-[#EF5DA8] inline-flex mr-2">
        <Button
          variant="ghost"
          size="sm"
      ...
      {unreadCount > 0 && (
        <span className="absolute top-0 right-0 translate-x-2 -translate-y-1 inline-flex items-center justify-center px-1.5 py-0.5 text-[10px] font-semibold leading-none text-white bg-linear-to-r from-[#5D5FEF] via-[#8A5AF1] to-[#EF5DA8] rounded-full">
          {unreadCount}
        </span>
      )}
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        {n.body && (
                          <div className="text-sm text-zinc-700">{n.body}</div>
                        )}
                      </div>
