...
export default function CreateEventPage() {
  ...
  const [selectedLocation, setSelectedLocation] = useState<SuggestItem | null>(null);
  ...
  useEffect(() => {
    const hasBasicInfo = formData.title && formData.date && formData.location.address;
    const hasTicketsInfo = formData.maxParticipants !== undefined || formData.requiresRSVP;
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      const eventData = {
        ...
        imageUrl: formData.imageUrl || 'https://images.unsplash.com/photo-1511578314322-379afb476865?q=80&w=1200&auto=format&fit=crop',
      };
      ...
      if (result.success) {
        ...
        const eventId = result.event.uuid || result.event.id || result.event._id;
  ...
  const getStepProgress = (step: Step): number => {
    ...
    if (step === 'tickets') {
      return formData.maxParticipants !== undefined || formData.requiresRSVP ? 100 : 0;
    }
    return 0;
  };
        ...
        <div className="w-64 bg-white border-r border-gray-200 fixed left-0 top-16 h-[calc(100vh-4rem)] overflow-y-auto z-10">
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              <Link href="/" className="flex items-center gap-2 text-gray-700 hover:text-[#5D5FEF]">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
                </svg>
                <span className="text-sm font-medium">Home</span>
              </Link>
              <Link href="/events" className="flex items-center gap-2 text-gray-700 hover:text-[#5D5FEF]">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {/* Back Link */}
            <Link href="/events" className="text-sm text-[#5D5FEF] hover:underline">
              ← Back to events
            </Link>
            ...
            <div className="space-y-4">
                  ...
                  <div className="relative w-6 h-6">
                    <svg className="w-6 h-6" viewBox="0 0 24 24">
                      ...
                      <circle
                        ...
                        strokeDashoffset={`${2 * Math.PI * 10 * (1 - getStepProgress('build') / 100)}`}
                        transform="rotate(-90 12 12)"
                      />
                    </svg>
                    {getStepProgress('build') === 100 && (
                      <svg className="absolute top-0 left-0 w-6 h-6 text-[#5D5FEF]" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                      </svg>
                    )}
                  </div>
                  <span className={`text-sm ${currentStep === 'build' ? 'font-semibold text-[#5D5FEF]' : 'text-gray-600'}`}>
                    Build event page
                  </span>
                </div>
                <p className="text-xs text-gray-500 ml-9">
                  Provide all the event info and give attendees a clear idea of what to expect.
                </p>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <div className="relative w-6 h-6">
                    <svg className="w-6 h-6" viewBox="0 0 24 24">
                      ...
                      <circle
                        ...
                        fill="none"
                        strokeDasharray={`${2 * Math.PI * 10}`}
                        strokeDashoffset={`${2 * Math.PI * 10 * (1 - getStepProgress('tickets') / 100)}`}
                        transform="rotate(-90 12 12)"
                      />
                    </svg>
                  ...
                  <span className={`text-sm ${currentStep === 'tickets' ? 'font-semibold text-[#5D5FEF]' : 'text-gray-600'}`}>
                    Add tickets
                  </span>
                </div>
                <p className="text-xs text-gray-500 ml-9">
                  Let attendees know how many people can join and how tickets work for your event.
                </p>
              </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  ...
                  <span className={`text-sm ${currentStep === 'publish' ? 'font-semibold text-[#5D5FEF]' : 'text-gray-600'}`}>
                    Publish
                  </span>
                </div>
                <p className="text-xs text-gray-500 ml-9">
                  Save as draft to finish later, publish to go live, or cancel to return to your dashboard.
                </p>
              </div>
            </div>
          </div>
                  ...
                  <div className="h-64 rounded-lg overflow-hidden border border-gray-200">
                    {process.env.NEXT_PUBLIC_GOOGLE_MAPS_API_KEY ? (
                      ...
                      <div className="w-full h-full bg-gray-100 flex items-center justify-center">
                        <p className="text-gray-400 text-sm">Map preview unavailable</p>
                      </div>
                    )}
                  </div>
                )}
            ...
            <Card className="border-[#5D5FEF]">
              ...
              <CardContent>
                <Textarea
                  ...
                  onChange={(e) => setFormData((prev) => ({ ...prev, lineup: e.target.value }))}
                  rows={4}
                />
              </CardContent>
            </Card>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        setFormData((prev) => ({ ...prev, requiresRSVP: e.target.checked }))
                      }
                      className="w-4 h-4"
                    />
                    <label htmlFor="requiresRSVP" className="text-sm text-gray-700">
                      Require RSVP
                    </label>
                  </div>
            ...
            <Card className="border-[#5D5FEF]">
              ...
              <CardContent>
                <p className="text-sm text-gray-600 mb-4">
                  Add tags or select a category to help people find your event more easily.
                </p>
